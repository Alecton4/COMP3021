...
public class FindMax {
	...
	public void printMax() throws InterruptedException {
		...
		for (int i = 0; i < 3; i++) {
			final int TMP = i; // !!! used in lambda
			threads[i] = new Thread(() -> {
				max_subArray[TMP] = findMax(30 * TMP, 29 + 30 * TMP);
			});
			threads[i].start();
		}
		...
		int idx_tmp = max_subArray[0] > max_subArray[1] ? 0 : 1;
		int max = max_subArray[idx_tmp] > max_subArray[2] ? max_subArray[idx_tmp] : max_subArray[2];
		System.out.println("the max value is " + max);
	}
